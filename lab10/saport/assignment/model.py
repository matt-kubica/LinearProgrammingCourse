...
class NormalizedAssignmentProblem:
    ...
    def from_problem(problem: AssignmentProblem) -> NormalizedAssignmentProblem:
        height, width = problem.n_workers(), problem.n_tasks()
        costs = problem.costs

        # invert costs if its a maximization problem
        if not problem.objective_is_min:
            costs = costs.max() - costs

        # filling array with zeros to form square
        # if array is higher than longer
        if height > width:
            costs = np.insert(costs, width, np.zeros(height), axis=1)
        # if array is longer than taller
        elif height < width:
            costs = np.insert(costs, height, np.zeros(width), axis=0)
        # if array is already square
        else: pass

        return NormalizedAssignmentProblem(costs, deepcopy(problem))
