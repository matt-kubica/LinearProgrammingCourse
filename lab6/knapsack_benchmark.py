...
class KnapsackBenchmark:
    ...
    def __init__(self, problems: List[str], timelimit: int = 500, solver_types: List[SolverType] = list(SolverType), print_function: Callable = print, problems_dir: str = "knapsack_problems"):
    ...
    def print_table(self, table_to_print):
        ...
        formatted_table_to_print = [
            [cell(v, longest_value) for v in row] for row in table_to_print]

        self.print_function('-' * (longest_value + 1) *
                            (len(table_to_print[0]) + 1))
    ...
    def run(self):
        ...
        for (i, p) in enumerate(self.problems):
            ...
            solvers = [SolverFactory.solver(
                st, problem, self.timelimit) for st in self.solver_types]
        ...
        ranking_header = ["<win ratio>"] + \
            [st.value for st in self.solver_types]
